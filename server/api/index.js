...
// Note: File uploads in Vercel serverless functions require special handling
// For production, consider using Vercel Blob Storage or external storage
app.post('/analysis/analyze', async (req, res) => {
  await initDB();
  try {
    // In Vercel, file uploads need to be handled differently
    // For now, we'll accept base64 encoded audio or use a mock
    const { setId, setName, audioData, audioDuration } = req.body;
    
    if (!setId) {
      return res.status(400).json({ error: 'Set ID is required' });
    }

    // Parse audio duration and exclusion times if provided
    const audioDurationSeconds = audioDuration ? parseInt(audioDuration, 10) : null;
    const excludeStartSeconds = excludeStart ? parseInt(excludeStart, 10) : 0;
    const excludeEndSeconds = excludeEnd ? parseInt(excludeEnd, 10) : 0;
    ...
    const analysis = await analyzeAudio(null, setData, audioDurationSeconds, excludeStartSeconds, excludeEndSeconds);
    ...
    const analysisDoc = {
      ...
      audio_file_name: 'uploaded_audio',
